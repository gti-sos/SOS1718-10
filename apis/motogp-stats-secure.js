...
exports.register = function(app, dbp, BASE_API_PATH_SECURE, checkApiKeyFunction) {

    ///FUNCION PAGINACION
    var insert = function(elementos, array, offset, limit) {
    ...
    app.get(BASE_API_PATH_SECURE + "/motogp-stats-secure/loadInitialData", function(req, res) {
    ...
    app.get(BASE_API_PATH_SECURE + "/motogp-stats-secure", (req, res) => {
        ...
        console.log(Date() + " - GET /motogp-stats-secure-secure");
        ...
        if (limit > 0 && offset >= 0) {
            console.log("INFO: New GET request to /motogp-stats-secure");
    ...
    /////////////////////////////////////////////////// GET a un recurso ////////////////////////////////////////////////////////////////////
    app.get(BASE_API_PATH_SECURE + "/motogp-stats-secure/:year", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        var year = req.params.year;
        if (!year) {
            console.log("WARNING: New GET request to /motogp-stats-secure/:year without season, sending 400...");
            res.sendStatus(400); /// bad request
        }
        else {
            console.log(Date() + " - GET /motogp-stats-sere/" + year);
            dbp.find({ "year": parseInt(year) }).toArray(function(err, filteredMotogpStats) {
                if (err) {
                    ...
                    if (filteredMotogpStats.length > 0) {
                        console.log(" INFO: Sending motogp-stats-secure: " + JSON.stringify(filteredMotogpStats[0], 2, null));
                        ...
                        console.log("WARNING: There are not any contact with motogp-stats-secure-secure" + year);
                        res.sendStatus(404); /// not found
                    }
                }
            });
    ...
    app.post(BASE_API_PATH_SECURE + "/motogp-stats-secure", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        var newPilot = req.body;
        if (!newPilot) {
            console.log("WARNING: New POST request to /motogp-stats-secure/ without motogp-stats-secure, sending 400...");
            res.sendStatus(400); /// bad request
        }
        else {
            console.log("INFO: New POST request to /motogp-stats-secure with body: " + JSON.stringify(newPilot, 2, null));
    ...
    app.put(BASE_API_PATH_SECURE + "/motogp-stats-secure", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        console.log(Date() + " - PUT /motogp-stats-secure");
        /// Method not allowed
        res.sendStatus(405);
    });

    ///////////////////////////////////// DELETE al conjunto de recursos /////////////////////////////////////////////////////////////////
    app.delete(BASE_API_PATH_SECURE + "/motogp-stats-secure", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        console.log(Date() + " - DELETE /motogp-stats-secure");
        dbp.remove({}, { multi: true }, function(err, result) {
            ...
            if (err) {
                ...
                if (numRemoved.n > 0) {
                    console.log("INFO: All the motogp-stats-secure (" + numRemoved + ") have been succesfully deleted, sending 204 ...");
    ...
    app.delete(BASE_API_PATH_SECURE + "/motogp-stats-secure/:year", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        var yearToRemove = req.params.year;
        if (!yearToRemove) {
            console.log("WARNIN: New GET request to /motogp-stats-secure/:year without season, sending 400...");
            res.sendStatus(400); /// bad request
        }
        else {
            console.log(Date() + " - DELETE /motogp-stats-secure/" + yearToRemove);
    ...
    app.post(BASE_API_PATH_SECURE + "/motogp-stats-secure/:year", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        var year = req.params.year;
        console.log(Date() + " - POST /motogp-stats-secure/" + year);
        res.sendStatus(405);
    });

    ///////////////////////////////////////// PUT A UN RECURSO (ACTUALIZA EL RECURSO) ///////////////////////////////////////////////////////
    app.put(BASE_API_PATH_SECURE + "/motogp-stats-secure/:year", (req, res) => {
        if(!checkApiKeyFunction(req,res))return;
        var year = req.params.year;
